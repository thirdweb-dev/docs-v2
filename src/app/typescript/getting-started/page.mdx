...
- Chain - The chain you want to connect to. This can be a chain name, chain id or a chain object from the `@thirdweb-dev/chains` package.
- Wallet - The wallet you want to use to connect to the chain. This can be a wallet instance from the Wallet SDK, a private key or a ethers.js Signer instance.
- SDK Options -
...
| -------------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Wallet (recommended) | [`ThirdwebSDK.fromWallet()`](/references/typescript/ThirdwebSDK#fromWallet)         | This function is async, needs to be awaited                                                        |
| ethers.js Signer     | [`ThirdwebSDK.fromSigner()`](/references/typescript/ThirdwebSDK#fromSigner)         | Use this when combining the SDK with other libraries                                               |
| Private Key          | [`ThirdwebSDK.fromPrivateKey()`](/references/typescript/ThirdwebSDK#fromPrivateKey) | This should only be used on the server side. Never commit or expose your private key on the client |

...
This will return an instance of [`SmartContract`](/references/typescript/SmartContract) which you can use to interact with the contract.

<Heading id="read" level={2} children="Reading State" />

Calling generic `view` functions on your contract can be done via the [`call`](/references/typescript/SmartContract#call) function. The return type will match the return type of the contract function.
...
Executing transactions on your contract can also be done via the [`call`](/references/typescript/SmartContract#call) function. The return type will always be a `TransactionResult` containing the transaction receipt.
...
Every write transaction in the SDK will throw an error of type [`TransactionError`](/references/typescript/TransactionError) if the execution fails for any reason.
...
You can also prepare a transaction without executing it. This will return a [`Transaction`](/references/typescript/Transaction) object that you can use to estimate, simulate, sign, send and execute the transaction yourself.
...
You can retrieve and listen to events emitted by your contract by using the [`contract.events`](typescript/ContractEvents) namespace.

...
Simulate the execution of a transaction to estimate the required gas limit.

```csharp
BigInteger estimatedGasLimit = await ThirdwebTransaction.Simulate(transaction);
```
...
Returns the estimated gas limit required for the transaction as a `BigInteger`.

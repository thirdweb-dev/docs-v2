...
## Configure the [ThirdwebProvider](/references/react-native/ThirdwebProvider)
...
## Connect to a wallet using our [ConnectWallet](/references/react-native/ConnectWallet) button
...
Connect to your smart contract using the [`useContract`](/references/react-native/useContract) hook like so:
...
You can then use [`useContractRead`](/references/react-native/useContractRead) and [`useContractWrite`](/references/react-native/useContractWrite) to read data and write transactions to the contract.

You pass the `contract` object returned from [`useContract`](/references/react-native/useContract) to these hooks as the first parameter and the name of the function (or view/mapping, etc.) on your smart contract as the second parameter. If your function requires parameters, you can pass them as additional arguments.
...
For example, if your smart contract implements [ERC721Supply](/contracts/build/extensions/ERC721Supply), you unlock the ability to view all NFTs on that contract using the SDK; which fetches all of your NFT metadata and the current owner of each NFT in parallel. In the React SDK, that is available using `useNFTs`:
...
If we want to mint an NFT and our contract implements [ERC721Mintable](/contracts/build/extensions/ERC721Mintable), we can use the [`useMintNFT`]references/react-native/useMintNFT) hook to mint an NFT from the connected wallet; handling all of the logic of uploading and pinning the metadata to IPFS for us behind the scenes.
...
We provide defaults for all of them, but you customize them to suit your needs. For more info see the [ThirdwebProvider reference](/references/react-native/ThirdwebProvider).

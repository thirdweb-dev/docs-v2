...
Show this wallet as "recommended" in the [ConnectWallet Modal](/react/components/ConnectWallet).
...
To allow users to connect to this wallet using the [ConnectWallet](/react/components/ConnectWallet) component, you can add it to [ThirdwebProvider's supportedWallets](/react-native/connecting-wallets#supported-wallets) prop.
...
You can use the [`useConnect`](/references/react-native/useConnect) hook to programmatically connect to the wallet without using the [ConnectWallet](/react/components/ConnectWallet) component.
...
`Chain` object corresponding to this chainId from [@thirdweb-dev/chains](https://www.npmjs.com/package/@thirdweb-dev/chains) package must be specified in [ThirdwebProvider's supportedWallets](/react-native/connecting-wallets#supported-wallets) prop as shown below

...
export const metadata = createMetadata({
	title: "Embedded Wallet | thirdweb React Native SDK",
	description: "API reference for embeddedWallet function",
});
...
A wallet configurator for [Embedded Wallet](/wallets/embedded-wallet/overview) which allows integrating the wallet with React Native
...
You can use the [`useEmbeddedWallet`](/references/react-native/v0/useEmbeddedWallet) hook to programmatically connect to the wallet without using the [ConnectWallet](/react-native/v0/connecting-wallets#connect-wallet) component.
...
You can delete an account by calling, `deleteActiveAccount` from the EmbeddedWallet instance:
...
function App() {
	...
	const deleteAccount = async () => {
		await (activeWallet as EmbeddedWallet).deleteActiveAccount();
	};
...
If you want to publish your app to the App Store you will need to allow users to delete their account. You
can use the `deleteActiveAccount` method for this purpose.

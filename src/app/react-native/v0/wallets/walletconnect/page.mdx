...
A wallet configurator for [Wallet Connect](/wallets/wallet-connect) which allows integrating the wallet with React Native
...
Show this wallet as "recommended" in the [ConnectWallet Modal](/react-native/v4/components/ConnectWallet).
...
To allow users to connect to this wallet using the [ConnectWallet](/react-native/v4/components/ConnectWallet) component, you can add it to [ThirdwebProvider's supportedWallets](/react-native/v4/connecting-wallets#supported-wallets) prop.
...
You can use the [`useConnect`](/references/react-native/v4/useConnect) hook to programmatically connect to the wallet without using the [ConnectWallet](/react-native/v4/components/ConnectWallet) component.

The wallet also needs to be added in [ThirdwebProvider's supportedWallets](/react-native/v4/connecting-wallets#supported-wallets) if you want the wallet to auto-connect on next page load.
...
`Chain` object corresponding to this chainId from [@thirdweb-dev/chains](https://www.npmjs.com/package/@thirdweb-dev/chains) package must be specified in [ThirdwebProvider's supportedWallets](/react-native/v4/connecting-wallets#supported-wallets) prop as shown below

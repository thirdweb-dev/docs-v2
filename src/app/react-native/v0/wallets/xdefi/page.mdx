...
export const metadata = createMetadata({
	title: "XDEFI | thirdweb React Native SDK",
	description: "API reference for xdefiWallet function",
});

<WalletsBreadcrumb name="XDEFI" slug="xdefi" />

# XDEFI

A wallet configurator for [XDEFI](/references/wallets/v2/XDEFIWallet) to integrate the wallet with the React Native SDK

```tsx
import { xdefiWallet } from "@thirdweb-dev/react-native";

const XDEFIConfig = xdefiWallet(options);
```

<CustomizeWalletConfigurator
	config="XDEFIConfig"
	configurator="xdefiWallet"
	connectUI="XDEFIConnectUI"
	selectUI="XDEFISelectUI"
/>
...
```tsx
<ThirdwebProvider supportedWallets={[xdefiWallet()]} clientId="your-client-id">
	<YourApp />
</ThirdwebProvider>
```
...
```tsx
const XDEFIConfig = xdefiWallet();

function App() {
	const connect = useConnect();

	const handleConnect = async () => {
		await connect(XDEFIConfig, connectOptions);
	};

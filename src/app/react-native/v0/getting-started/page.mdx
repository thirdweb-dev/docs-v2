...
If we want to mint an NFT and our contract implements [ERC721Mintable](/contracts/build/extensions/ERC721Mintable), we can use the [`useMintNFT`](/references/react-native/v0/useMintNFT) hook to mint an NFT from the connected wallet; handling all of the logic of uploading and pinning the metadata to IPFS for us behind the scenes.

...
Hook to prompt the user to sign in with their wallet using [Auth](/wallets/auth).

Requires the [`authConfig`](/references/react/v4/ThirdwebProviderCoreProps#authConfig) prop to be configured
on the [`ThirdwebProvider`](/react-native/v0/ThirdwebProvider).

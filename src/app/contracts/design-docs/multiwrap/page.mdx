...
export const metadata = createMetadata({
	...
	title: "Multiwrap design document | thirdweb",
	description:
		"This is a live document that explains what the thirdweb Multiwrap smart contract is, how it works and can be used, and why it is designed the way it is",
});
...
<DocImage src={multiwrapDiagramImage} alt="Multiwrap diagram" />
...
We're building `Multiwrap` for cases where an application wishes to bundle up / distribute / transact over _n_ independent tokens all at once, as a single asset. This opens up several novel NFT use cases.

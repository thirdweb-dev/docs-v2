...
export const metadata = createMetadata({
	title: "GetSwapStatus | thirdweb Unity SDK",
	description: "Get swap status for a transaction hash.",
});

# GetSwapStatus

Get swap status for a transaction hash returned by [SendSwap](/unity/pay/sendswap).
...
{
	SwapStatusResult status = await ThirdwebPay.GetSwapStatus(_txHash);
...
`string`: The transaction hash of the swap transaction returned by [SendSwap](/unity/pay/sendswap).

### Return Value

A SwapStatusResult object containing the following properties:
...
string toAddress; // The destination address.

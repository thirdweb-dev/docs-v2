...
export const metadata = createMetadata({
	title: "GetSwapHistory | thirdweb Unity SDK",
	description: "Get swap history, supports cursor and pagination.",
});

# GetSwapHistory

Get swap history for a specific wallet that [executed a swap](/unity/pay/sendswap), supports cursor and pagination.
...
public async void GetSwapHistory()
{
	...
	SwapHistoryResult history = await ThirdwebPay.GetSwapHistory(walletAddress: connectedAddress, start: 0, count: 1);
	ThirdwebDebug.Log($"History: {JsonConvert.SerializeObject(history, Formatting.Indented)}");

	SwapHistoryResult historyNext = await ThirdwebPay.GetSwapHistory(walletAddress: connectedAddress, start: 1, count: 10, cursor: history.NextCursor, pageSize: null);
...
A SwapHistoryResult object containing the following properties:

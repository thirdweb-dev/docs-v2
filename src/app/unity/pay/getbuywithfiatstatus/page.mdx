...
In some cases, you may onramp to an intermediary token that requires an additional step, in these cases the `PENDING_CRYPTO_SWAP` status indicates that the onramp transfer has completed and you may now use the intent id to trigger a [BuyWithCrypto](/unity/pay/buywithcrypto) transaction to get to your destination token. The flow is the same for [GetBuyWithCryptoQuote](/unity/pay/getbuywithcryptoquote), though you may now pass the intent id as a parameter to link both statuses together.

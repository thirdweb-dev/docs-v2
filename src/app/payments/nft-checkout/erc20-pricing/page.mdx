...
2. Configure your checkout to accept USDC payment:
   ...
   2. **One-Time Checkout Links:** When [creating a One-Time Checkout Link](/payments/one-time-checkout-link) set `mintMethod` with `"payment": { "value": "50 * $QUANTITY", currency: "USDC" }`.
   3. **Checkout Elements:** When [creating a Client Secret](/payments/elements) set `mintMethod` with `"payment": { "value": "50 * $QUANTITY", currency: "USDC" }`.

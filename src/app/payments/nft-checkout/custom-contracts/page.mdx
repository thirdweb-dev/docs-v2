...
3. (If priced in USDC) The method must explicitly [request USDC token](/payments/erc20-pricing) from the `msg.sender`.

## MintMethod

The **mintMethod** is required when creating [Shareable Checkout Links](/payments/checkout-link), [One-Time Checkout Links](/payments/one-time-checkout-link), or [Checkout Elements](/payments/elements). Think of it as an ABI for thirdweb to know how to call your claim function.

...
For some thirdweb contracts, set `contractArgs` when creating [Shareable Checkout Links](/payments/checkout-link), [One-Time Checkout Links](/payments/one-time-checkout-link), or [Checkout Elements](/payments/elements).

### Signature Drop

This is an ERC-721A contract where the NFT metadata is unique but the claim configuration can be modified for each buyer by creating a _signature_ on your backend. If you don't plan to use signatures, you should consider using the [NFT Drop](/pre-built-contracts/nft-drop) contract.

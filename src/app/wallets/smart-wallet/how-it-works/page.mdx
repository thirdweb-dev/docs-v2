...
export const metadata = createMetadata({
	...
	title: "What is a Smart Wallet? | thirdweb",
	description:
		"Smart Wallet is a wallet that is controlled by a smart contract following the ERC-4337 specification",
});
...
This smart wallet is unlocked by a 'key' - a personal account. This key can be anything from a [MetaMask](/references/wallets/v2/MetaMaskWallet) wallet or even a [Local Wallet](/references/wallets/v2/LocalWallet) and is used as a way to 'sign in' to the wallet.
...
| -------------------------------------------------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`Simple`](https://thirdweb.com/thirdweb.eth/AccountFactory)         | Non-upgradeable                                             | Developer wants to issue simple smart wallets to their users. They do not anticipate that users wallets will need any feature upgrades.                                                       |
| [`Dynamic`](https://thirdweb.com/thirdweb.eth/DynamicAccountFactory) | Account upgrades controlled locally by each account.        | Developer wants to issue smart wallets to their users. They do anticipate feature upgrades to user wallets but do not want to push upgrades to user wallets without consent.                  |
| [`Managed`](https://thirdweb.com/thirdweb.eth/ManagedAccountFactory) | Account upgrades controlled centrally by the app developer. | Developer wants to issue smart wallets to their users. They do anticipate feature upgrades to user wallets, and want to push upgrades to user wallets for seamless/invisible UX for upgrades. |
...
can be used during the call execution via the `EntryPoint` contract. The thirdweb [Smart Wallet](/wallets/smart-wallet) paymaster is initially paid for by thirdweb. By using an API key,
the usage is tracked and billed. You can easily define [sponsorship rules](/wallets/smart-wallet/sponsorship-rules) that determine whether a transaction will be sponsored.

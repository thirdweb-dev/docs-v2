...
By using the [wallet SDK](/references/wallets) alongside the [TypeScript SDK](/typescript), you can use smart wallets in your applications easily.
...
If you are working in a React environment, you are recommended to follow [this guide](/wallets/smart-wallet/guides/react).
...
This key can be anything from a [MetaMask wallet](/references/wallets/MetaMaskWallet) or even a [Local Wallet](/references/wallets/LocalWallet) and is used as a way to 'sign in' to the wallet.
...
To create a personal wallet key, we are going to use the [Local Wallet](/references/wallets/LocalWallet), which we need to import from the `@thirdweb-dev/wallets` package, and store it in an ecrypted JSON file.

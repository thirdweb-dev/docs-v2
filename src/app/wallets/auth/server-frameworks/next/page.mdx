import { OpenSourceCard, CardLink, Callout } from "@/components/Document";

# Next

This page will focus mainly on integrating Auth on the server-side with [Next.js](https://nextjs.org) by implementing the [Auth API](/wallets/auth/how-it-works/api).

To learn more about how to implement Auth on the client-side with Next.js (which you will likely need after implementing the server-side), see the Auth [React page](/wallets/auth/client-frameworks/react) as Next uses React for the frontend:

<CardLink
	label="Auth & React integration"
	href="/wallet/auth/client-frameworks/react"
	icon="educate"
/>
...
<OpenSourceCard
	repoName="thirdweb-auth-next"
	repoUrl="https://github.com/thirdweb-example/thirdweb-auth-next"
...
To add support for Auth to your Next backend, you'll need to install the `@thirdweb-dev/auth` package (as well as any other packages you'll need for your [wallet configuration](/wallets/auth/wallet-configuration), we'll need the `ethers` package for this example as we'll be using the `PrivateKeyWallet`):

```bash npm2yarn
npm install @thirdweb-dev/auth ethers@5
```

Now we can setup our Next.js server by using the `@thirdweb-dev/auth/next` entrypoint. First, we need to setup our [Auth API](/wallets/auth/hoa-it-works/api) by creating a new file called `pages/api/auth/[...thirdweb].ts`:
...
<CardLink
	...
	href="/wallets/auth/wallet-configuration"
...
By default, the Auth API will validate the login request by checking that the user requesting to login successfully signed a valid [sign-in with wallet](/wallets/auth/how-it-works/siwe) message. However, this doesn't perform specific checks on the exact contents of the payload, aside from the domain used for anti-phishing.
...
When the server issues a JWT, the default JWT claims are added to the token (as elaborated in the [how auth works section](/wallets/auth/how-it-works/json-web-tokens)). However, you may want to add additional data to the JWT claims to serve as session data that will persist on the token, such as certain user level permissions or access information. You can do this by returning data from the `callbacks.onLogin` callback function which treats your return value as session data to store onto the JWT.
...
| -------------------------------------- | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `domain`                               | ✅       | -        | The domain of your app, used for anti-phishing. See the [sign-in with wallet](/wallets/auth/hoa-it-works/siwe) overview for more details.                                                                                                                                                                    |
| `wallet`                               | ✅       | -        | The wallet used to issue and verify JWTs. See the [wallet configuration](/wallets/auth/wallet-configuration) page for more details.                                                                                                                                                                          |
| `authOptions`                          | ❌       | -        | Configuration used to validate login requests and JWTs                                                                                                                                                                                                                                                       |
| `authOptions.statement`                | ❌       | -        | The required statement used on the login payload. See the [sign-in with wallet](/wallets/auth/hoa-it-works/siwe) overview for more details.                                                                                                                                                                  |
| `authOptions.uri`                      | ❌       | -        | The required URI used on the login payload. See the [sign-in with wallet](/wallets/auth/hoa-it-works/siwe) overview for more details.                                                                                                                                                                        |
| `authOptions.version`                  | ❌       | `1`      | The required version used on the login payload. See the [sign-in with wallet](/wallets/auth/hoa-it-works/siwe) overview for more details.                                                                                                                                                                    |
| `authOptions.chainId`                  | ❌       | -        | The required chain ID used on the login payload. If set, smart contract wallets on other chains won't be able to login. See the [sign-in with wallet](/wallets/auth/hoa-it-works/siwe) overview for more details.                                                                                            |
| `authOptions.resources`                | ❌       | -        | The required resources used on the login payload. See the [sign-in with wallet](/wallets/auth/hoa-it-works/siwe) overview for more details.                                                                                                                                                                  |

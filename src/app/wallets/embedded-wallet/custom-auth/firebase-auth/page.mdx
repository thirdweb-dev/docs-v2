import { Callout, DocImage, Steps, Step } from "@doc";
...
You can use the following function to extract your encryption key:

```jsx
export function getEncryptionKey(jwt: string): string {
  ...
  const decodePayload = JSON.parse(Buffer.from(encodedPayload, "base64").toString("utf8"))

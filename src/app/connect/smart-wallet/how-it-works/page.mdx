...
can be used during the call execution via the `EntryPoint` contract. The thirdweb [Smart Wallet](/connect/smart-wallet) paymaster is initially paid for by thirdweb. By using an API key,
the usage is tracked and billed. You can easily define [sponsorship rules](/connect/smart-wallet/sponsorship-rules) that determine whether a transaction will be sponsored.

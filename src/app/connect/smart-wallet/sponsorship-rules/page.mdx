...
You can easily set sponsorship rules by going to the [configuration tab in the Account abstraction dashboard.](https://thirdweb.com/dashboard/connect/account-abstraction).To use them in your app all you need to do is pass your API key, sponsorship rules will automatically be applied.
...
Sponsorship rules apply across all the [networks that we support account abstraction in](https://portal.thirdweb.com/wallets/smart-wallet/infrastructure#supported-chains). You don't need to specify different rules for each network or each account factory as long as they use the same client ID.

...
const USDC = getContract({ client, address: USDC_ADDRESS, chain: ethereum });
...
export default function MyComponent() {
  ...
  const onClick = () => {
    // send 1500 USDC to the target address
    const transaction = transfer({ contract: USDC, amount: 1500 , to: "0x..." });

...
import getStartedImage1 from "./../assets/getting-started-1.png";
import getStartedImage2 from "./../assets/getting-started-2.png";
import getStartedImage3 from "./../assets/getting-started-3.png";
...
export const metadata = createMetadata({
	...
	title: "Get started with Auth | thirdweb",
	description:
		"we'll walk through the process of adding Auth to a simple full-stack application to enable wallet-based authentication",
});
...
Specifically, we'll be integrating Auth with a [Next.js](https://nextjs.org) app, which has its own built-in API, as well as a [React](https://reactjs.org) frontend. With this setup, we can add [Auth API](/connect/auth/how-it-works/api) endpoints to our app to handle authentication and authorization, and then we can build a simple frontend to login, display the user, and logout.

If you want to see the follow-along with the full code for this guide, we'll be using the following example repository:

<OpenSourceCard
	title="Auth + Next.js Example"
	href="https://github.com/thirdweb-example/thirdweb-auth-next"
	description="A simple example of using Auth with Next.js."
/>
...
Now that we have the repository set, we can continue to looking through and understand the relevant code.
...
This file is a Next.js catch-all route, which will catch any requests to `/api/auth/*` endpoints. The `ThirdwebAuth` function takes in configuration for your Auth API. It has many optional parameters that aren't used here (you can read more about them on the [Auth Next.js page](/connect/auth/server-frameworks/next)), but the two mandatory parameters are `domain` and `wallet`.

`domain` is used to prevent phishing attacks on your users - you can learn more about it in the [how auth works section](/connect/auth/how-it-works/siwe), but the important thing is that this `domain` matches the `domain` we will later configure on the frontend.
...
The `ThirdwebAuth` function returns us a `ThirdwebAuthHandler`, which automatically sets up multiple authentication endpoints for us to use that handle logins, authentication, and logouts (as laid out in the [Auth API overview](/connect/auth/how-it-works/api)). It also returns a `getUser` function, which we can use on other endpoints to authenticate the user that sent a request.
...
First, let's look at the `pages/_app.tsx` file. Here, we want to setup the [`@thirdweb-dev/react`](/react/latest) SDK to use with the Auth package. To do this, we need to wrap our app with the `ThirdwebProvider` component and add `authConfig` to it so that our frontend knows that we have an Auth backend setup.
...
If you run the app and navigate to `http://localhost:3000`, or wherever the app is running for you locally, you should see a simple connect wallet button. If you click on it an connect your MetaMask, you'll see a screen like this:

<DocImage className="w-[550px]" src={getStartedImage1} />
...
<DocImage className="w-[550px]" src={getStartedImage2} />
...
<DocImage className="w-[550px]" src={getStartedImage3} />

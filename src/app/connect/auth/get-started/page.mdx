...
Specifically, we'll be integrating Auth with a [Next.js](https://nextjs.org) app, which has it's own build in API, as well as a [React](https://reactjs.org) frontend. With this setup we can add [Auth API](/connect/auth/how-it-works/api) endpoints to our app to handle authentication and authorization, and then we can build a simple frontend to login, display the user, and logout.
...
If your run the app and navigate to `http://localhost:3000`, or wherever the app is running for you locally, you should see a simple connect wallet button. If you click on it an connect your MetaMask, you'll see a screen like this:

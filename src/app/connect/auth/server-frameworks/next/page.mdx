...
We can setup the server with the `pages` directory by using the `@thirdweb-dev/auth/next` entrypoint. First, we need to setup our [Auth API](/connect/auth/how-it-works/api) by creating a new file called `pages/api/auth/[...thirdweb].ts`:

...
To skip over the initialization steps, we will use the React Native TypeScript starter template. You may also integrate this functionality into an existing application by installing the React Native SDK.
...
```jsx
import { useInAppWallet } from "@thirdweb-dev/react-native";

const connectInApp = useInAppWallet();

...
In React and React Native, the `useInAppWallet()` hook handles authentication and connection states.

```typescript
import { useInAppWallet } from "@thirdweb-dev/react"; // or /react-native
...
const inAppWallet = new InAppWallet({
	chain: Goerli, //  chain to connect to
	clientId: "YOUR_CLIENT_ID", // Your thirdweb client ID
});

const authResult = await inAppWallet.authenticate({
	strategy: "jwt",
	jwt,
	encryptionKey: "your-encryption-key",
});

const walletAddress = await inAppWallet.connect({ authResult });
```

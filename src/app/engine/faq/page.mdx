...
Cancel a transaction with [`POST /transaction/cancel`](https://redocly.github.io/redoc/?url=https://demo.web3api.thirdweb.com/json#tag/Transaction/operation/cancel).

If a transaction is queued, this will remove it from the queue.
If a transaction is already submitted to mempool, this will send a null transaction with aggressive gas options to "replace" the original transaction.
This will submit a null tx
If a transaction is has not yet been mined or failed, cancel the

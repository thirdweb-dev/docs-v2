...
1. Using the [`ConnectWallet`](#connect-wallet) component for a quick, easy and customizable UI
...
[`ConnectWallet`](/react/components/ConnectWallet) component renders a button which when clicked opens a modal to allow users to connect to wallets specified in the [ThirdwebProvider's supportedWallets](/react/ThirdwebProvider#supportedWallets) prop.
...
- [MetaMask](/references/react/metamaskWallet)
- [Coinbase Wallet](/references/react/coinbaseWallet)
- [WalletConnect](/references/react/walletConnect)
- [Rainbow](/references/react/rainbowWallet)
- [Trust Wallet](/references/react/trustWallet)
- [Zerion Wallet](/references/react/zerionWallet)
- [Phantom](/references/react/phantomWallet)
...
If you just want to connect a wallet as mentioned above - there's a simple way to do it using the [useConnect](/references/react/useConnect) hook which does all the above steps for you.
...
you can do these steps manually as shown below using the [useCreateWalletInstance](/references/react/useCreateWalletInstance), [useSetConnectionStatus](/references/react/useSetConnectionStatus) and [useSetConnectedWallet](/references/react/useSetConnectedWallet) hooks.
...
We will use [`localWallet`](/references/react/localWallet) as an example because it can not be connected using the `useConnect` hook because it requires additional steps after creating the wallet instance - like generating a random wallet or importing a wallet. We will generate a random wallet in this example.

import { Details, Breadcrumb, Callout, Steps, Step } from "@doc";
...
It also renders a UI to [Sign in with wallet](/wallets/auth/how-it-works/siwe) if it is enabled by setting [`authConfig`](/react/v4/ThirdwebProvider#authConfig) in [`ThirdwebProvider`](/react/v4/ThirdwebProvider) component and `loginOptional` prop is either not provided or set to `false` in `ConnectEmbed` component
...
Enforce that users must [sign in with their wallet](/wallets/auth/how-it-works/siwe) using [auth](/wallets/auth) after connecting their wallet.

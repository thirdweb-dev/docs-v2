...
- An [access token](/infrastructure/engine/features/authentication) for your Engine instance
...
Use [`<ConnectWallet>`](/react/components/ConnectWallet) to prompt the buyer for their wallet address. The buyer provides their credit card details and selects **Pay now** to send payment details directly to Stripe.
